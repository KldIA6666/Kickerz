# ===================================================================================
# Project: KickerzViewsHybrid
# Version: 12.0 (Concurrency Controlled & Feature Complete)
# Description: The definitive high-performance version. Re-implements the concurrency
#              limiter (Semaphore) to prevent system overload. Advanced anti-detection
#              features are now optional for maximum flexibility and speed.
#
...
# Controls how many workers are active at once. Prevents system overload.
...
# --- Resilience & Looping Configuration ---
MAX_RETRIES = 3
...
thread_states = {}; worker_types = {}
successful_visits = 0; failed_visits = 0
...
class BrowserManager:
    ...
    def __init__(self, playwright_instance, proxy_config=None):
        self.browser = None; self._playwright = playwright_instance; self._proxy_config = proxy_config
    ...
    async def new_context(self):
        ...
        return await self.browser.new_context(ignore_https_errors=True, user_agent=USER_AGENT, viewport=viewport)
...
def rich_dashboard_thread(stop_event: threading.Event, run_mode: str, total_workers: int, proxy_workers: int):
    """Renders the definitive UI with concurrency and detailed status info."""
    def generate_layout() -> Panel:
        with state_lock:
            ...
            config_table.add_row(f"Mode: [bold magenta]{run_mode}[/]"); config_table.add_row(f"Total Workers: [bold cyan]{total_workers}[/]")
            ...
            active_sessions = status_counts.get('Navigating', 0) + status_counts.get('On Page', 0)
            
            summary_table = Table.grid(expand=True)
            summary_table.add_row(Text(f"⚡ Active Sessions: {active_sessions} / {MAX_CONCURRENT_SESSIONS}", style="green"))
            ...
            summary_table.add_row(Text(f"⏳ Waiting Queue:   {status_counts.get('Waiting', 0)}", style="yellow"))
            summary_table.add_row(Text(f"❌ Failed / Ended:  {status_counts.get('Failed', 0)}", style="red"))

        performance_panel = Panel(perf_table, title="[bold yellow]Performance[/bold yellow]", border_style="yellow")
        config_panel = Panel(config_table, title="[bold yellow]Configuration[/bold yellow]", border_style="yellow")
        summary_panel = Panel(summary_table, title="[bold blue]Worker Summary[/bold blue]", border_style="blue")
...
async def perform_human_like_actions(page):
    ...
    try:
        ...
        for _ in range(random.randint(1, 3)):
            ...
            await page.mouse.wheel(0, random.randint(200, min(scroll_height, 1000))); await asyncio.sleep(random.uniform(0.5, 1.5))
    except Exception as e:
        logging.warning(f"Could not perform human-like actions: {e}")
...
async def visit_website_headless(manager: BrowserManager, target_url: str, visit_duration: int, worker_id: int, use_advanced_actions: bool):
    """Performs a single, isolated visit."""
    ...
    try:
        ...
        for _ in range(MAX_RETRIES):
            try:
                ...
                continue
        with state_lock: failed_visits += 1; update_thread_state(worker_id, "Failed")
        logging.error(f"Worker-{worker_id} failed to view {target_url} after {MAX_RETRIES} retries.")
    finally:
        if context: await context.close()

async def worker_loop(semaphore: asyncio.Semaphore, manager: BrowserManager, url_list: list, visit_duration: int, worker_id: int, use_advanced_actions: bool):
    """The main worker loop, controlled by the semaphore for concurrency."""
    await asyncio.sleep(random.uniform(0, 5)) # Stagger initial start
    while True:
        update_thread_state(worker_id, "Waiting")
        async with semaphore:
            # This worker has acquired a pass. It is now "Active".
            try:
...
async def check_proxy(proxy_config):
    """Performs a pre-run health check on proxy credentials."""
...
async def main(proxy_workers: int, direct_workers: int, urls: list, duration: int, run_mode: str, advanced_actions: bool):
    """Initializes managers and runs all workers as asyncio tasks under a semaphore."""
    ...
    async with async_playwright() as p:
        ...
        try:
            if proxy_workers > 0: proxy_manager = BrowserManager(p, proxy_config); await proxy_manager.launch()
            if direct_workers > 0: direct_manager = BrowserManager(p); await direct_manager.launch()

            tasks = []
            for i in range(1, proxy_workers + 1):
                worker_types[i] = 'Proxy'; tasks.append(asyncio.create_task(worker_loop(semaphore, proxy_manager, urls, duration, i, advanced_actions)))
            for i in range(proxy_workers + 1, total_workers + 1):
                worker_types[i] = 'Direct'; tasks.append(asyncio.create_task(worker_loop(semaphore, direct_manager, urls, duration, i, advanced_actions)))
...
if __name__ == "__main__":
    try:
        ...
        duration_input = int(console.input("[bold]Enter visit duration in seconds (on-page time): [/]").strip())
        
        console.print("\n[bold]Enable advanced human-like actions? (slower, more realistic)[/]")
        adv_choice = console.input("[bold](y/n): [/]").strip().lower()
        advanced_actions_enabled = True if adv_choice == 'y' else False

        console.print("\n[bold]Select a connection mode:[/]")
        console.print("  [cyan]1.[/] Proxies Only"); console.print("  [cyan]2.[/] Direct Connections Only"); console.print("  [cyan]3.[/] Hybrid Mode")
        mode_choice = console.input("> ").strip()

        proxy_w, direct_w, run_mode_str = 0, 0, ""
        if mode_choice == '1':
            run_mode_str = "Proxy Only"; proxy_w = int(console.input("Enter number of [blue]proxy[/] workers: ").strip())
        elif mode_choice == '2':
            run_mode_str = "Direct Only"; direct_w = int(console.input("Enter number of [blue]direct[/] workers: ").strip())
        elif mode_choice == '3':
            run_mode_str = "Hybrid Mode"; console.print("[dim]Note: Proxy providers often have connection limits.[/dim]")
            proxy_w = int(console.input("Enter number of [blue]proxy[/] workers: ").strip())
            direct_w = int(console.input("Enter number of [blue]direct[/] workers: ").strip())
        ...
        logging.info(f"Starting KickerzViews. Mode: {run_mode_str}, Proxies: {proxy_w}, Direct: {direct_w}, URLs: {urls_to_visit}, Advanced Actions: {advanced_actions_enabled}")
    ...
    except (ValueError, TypeError): console.print(f"\n[bold red]Error: Invalid input.[/]")
    except KeyboardInterrupt: console.print("\n\n[bold yellow]Keyboard interrupt detected. Shutting down...[/]")
    finally:
