...
# Version: 8.0 (Pure Async Architecture)
# Description: A high-performance, purely asynchronous script using a single-browser,
#              multi-context model. Eliminates thread deadlocks for true scalability
#              and reliability. The UI is a summary-based dashboard.
#
...
class BrowserManager:
    ...
    def __init__(self, playwright_instance, proxy_config):
        self.browser = None
...
async def worker_loop(manager: BrowserManager, target_url: str, visit_duration: int, worker_id: int):
    ...
    while True:
        try:
            ...
            # This is raised when the script is shutting down
            break
        except Exception:
            update_thread_state(worker_id, "Failed")
...
async def main(total_workers: int, url: str, duration: int):
    ...
    console.print(f"Architecture: [bold magenta]Pure Async / Single Browser (High-Scale Model)[/]")
    ...
    async with async_playwright() as p:
        manager = BrowserManager(p, proxy_config)
        ...
        try:
            for i in range(1, total_workers + 1):
                ...
                # Create a task for each worker. They will start running concurrently.
                task = asyncio.create_task(worker_loop(manager, url, duration, i))
                tasks.append(task)
            
            # Wait for all tasks to complete (which they won't until cancelled by Ctrl+C)
            ...
            await manager.close()
            stop_ui_event.set() # Signal the UI thread to stop
...
if __name__ == "__main__":
    try:
        ...
        if PROXY_TOKEN == "YOUR_PROXY_TOKEN" or PROXY_SERVER == "YOUR_PROXY_SERVER_ADDRESS:PORT":
            console.print("\n[bold yellow]WARNING:[/] Proxy details are not set.", style="yellow")
            time.sleep(3)
